...
def get_wind_data(start_date, end_date, unit_id):
    ...
    for date in date_range:
        url = 'https://api.bmreports.com/BMRS/B1610/v2?APIKey={}&SettlementDate={}&Period=*&ServiceType=csv&NGCBMUnitID={}'.format(
            API_KEY, date, unit_id)
...
def get_demand_data(start_date, end_date):
    ...
    for date in date_range:
        url = 'https://api.bmreports.com/BMRS/SYSDEM/v1?APIKey={}&FromDate={}&ToDate={}&ServiceType=csv'.format(API_KEY,
                                                                                                                date,
                                                                                                                date)
        response = requests.get(url, allow_redirects=True)
