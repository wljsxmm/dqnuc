...
class Env(object):
    ...
    def __init__(self, gen_info, profiles_df,
        ...
        # Max cost per mwh
        self.gen_info['max_cost_per_mwh'] = (self.a * (
                self.min_output ** 2) + self.b * self.min_output + self.c) / self.min_output
        ...
        # Set up for outages  停机率  —— 可以设置对停机的鲁棒
        ...
        self.weibull = True  # use Weibull by default 韦伯分布（Weibull distribution）是可靠性分析和寿命检验的理论基础
        self.weibull_loc = 0
        self.weibull_scale = 100

        # Set up for curtailment 削减
    ...
    def update_gen_status(self, action):
        """ Subroutine updating generator statuses. 更新生成器状态的子程序"""
    ...
    def is_feasible(self):
        ...
        horizon = max(0, np.max((self.t_min_down + self.status)[np.where(
            self.commitment == 0)]))  # Get the max number of time steps required to determine feasibility
        horizon = min(horizon, self.episode_length - self.episode_timestep)  # Horizon only goes to end of day

        for t in range(1, horizon):
            net_demand = self.episode_forecast[self.episode_timestep + t] - self.episode_wind_forecast[
                self.episode_timestep + t]  # Nominal demand for t+1th period ahead
            future_status = self.status + (t) * np.where(self.status > 0, 1,
                                                         -1)  # Assume all generators are kept on where possible

            available_generators = np.logical_or((-future_status >= self.t_min_down),
                                                 self.commitment)  # Determines the availability of generators as binary array
...
def interpolate_profile(profile, upsample_factor):
    """
    Interpolate （通过插补进行插补） a demand/renewables profile, upsampling by a factor of
    upsample_factor
    """
...
def make_env_from_json(env_name='5gen', mode='train', profiles_df=None):
    """
    Create an environment object.
    如果在主函数里没传参 就默认用这里定义的参数
    """
    script_dir = os.path.dirname(os.path.realpath(__file__))
    env_fn = os.path.join(script_dir, 'data/envs/{}.json'.format(env_name))  # 指定机组环境 不需要加后缀名 json
