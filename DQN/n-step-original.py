...
def train(buffer, target_model, eval_model, gamma, optimizer, batch_size, loss_fn, count, soft_update_freq, n_step):
    ...
    t1 = eval_model.forward(next_observation)
    t2 = eval_model.forward(next_observation).max(1)
    argmax_actions = eval_model.forward(next_observation).max(1)[1].detach()

    t3 = argmax_actions.unsqueeze(1)
    t4 = next_q_values.gather(1, argmax_actions.unsqueeze(1))
...
if __name__ == '__main__':
    ...
    for i in range(episode):
        ...
        if render:
            env.render()
        while True:
            a = torch.FloatTensor(np.expand_dims(obs, 0))
